...
@ViewScoped
@ManagedBean(name="cadastrarAtendimentoBean")
public class CadastrarAtendimentoBean extends AbstractBean implements Serializable {
	...
	public void init() {
		...
		Integer id = Integer.valueOf(params.get("idCidadao"));
		
		if(id != null) {
			try {
				cidadao = new CidadaoService().ObterPorId(id);
			} catch (Exception e) {
				e.printStackTrace();
			}
		}	
